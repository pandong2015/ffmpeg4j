...
以 Spring 官方约定的 autoconfigure + starter 两模块分层，把 core 的环境发现、执行引擎与门面 `FfmpegClient` 自动装配为 Spring bean：支持 `@ConfigurationProperties` 强类型配置绑定、显式二进制路径优先否则 PATH 发现、启动期 fail-fast 校验，并以 `@ConditionalOnMissingBean` 让用户 bean 覆盖优先。core 保持零重型运行时依赖不变，所有 Spring 依赖只落在 autoconfigure/starter。
